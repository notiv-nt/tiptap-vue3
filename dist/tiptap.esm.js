...
class ComponentView {
  constructor(component, { editor, extension, parent, node, view, decorations, getPos }) {
...
class ExtensionManager {
  ...
  get nodes() {
    return this.extensions.filter((extension) => extension.type === 'node').reduce((nodes, { name, schema }) => ({ ...nodes, [name]: schema }), {});
  }
  ...
  get marks() {
    return this.extensions.filter((extension) => extension.type === 'mark').reduce((marks, { name, schema }) => ({ ...marks, [name]: schema }), {});
  }

  get plugins() {
    return this.extensions.filter((extension) => extension.plugins).reduce((allPlugins, { plugins }) => [...allPlugins, ...plugins], []);
  }
  ...
  inputRules({ schema, excludedExtensions }) {
    ...
    const allowedExtensions =
      excludedExtensions instanceof Array ? this.extensions.filter((extension) => !excludedExtensions.includes(extension.name)) : this.extensions;
...
class Editor extends Emitter {
  ...
  initNodeViews({ parent, extensions }) {
    return extensions
      ...
      .reduce((nodeViews, extension) => {
        ...
        return { ...nodeViews, [extension.name]: nodeView };
      }, {});
  }
  ...
  setActiveNodesAndMarks() {
    this.activeMarks = Object.entries(this.schema.marks).reduce(
      (marks, [name, mark]) => ({ ...marks, [name]: (attrs = {}) => markIsActive(this.state, mark, attrs) }),
      {}
    );
    this.activeMarkAttrs = Object.entries(this.schema.marks).reduce(
      (marks, [name, mark]) => ({ ...marks, [name]: getMarkAttrs(this.state, mark) }),
      {}
    );
    this.activeNodes = Object.entries(this.schema.nodes).reduce(
      (nodes, [name, node]) => ({ ...nodes, [name]: (attrs = {}) => nodeIsActive(this.state, node, attrs) }),
      {}
    );
  }
  ...
  get isActive() {
    return Object.entries({ ...this.activeMarks, ...this.activeNodes }).reduce(
      (types, [name, value]) => ({ ...types, [name]: (attrs = {}) => value(attrs) }),
      {}
    );
  }
...
class Menu$1 {
  ...
  update(view, lastState) {
    ...
    const { from, to } = state.selection; // These are in screen coordinates
...
class Menu$2 {
  ...
  update(view, lastState) {
    const { state } = view; // Don't do anything if the document/selection didn't change
